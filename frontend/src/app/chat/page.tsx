...
export default function ChatPage() {
  ...
  // ═══ SÉLECTIONNER LA SESSION DEPUIS L'URL ═══
  useEffect(() => {
    if (sessionIdFromUrl && sessions.length > 0) {
      setSelectedSessionId(sessionIdFromUrl);
    } else if (!sessionIdFromUrl && sessions.length > 0 && !selectedSessionId) {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => newSocket.emit('get_history', { session_id: selectedSessionId }));
    newSocket.on('history', (data) => {
      if (data.success) { setMessages(data.messages || []); setChatLoading(false); setTimeout(() => scrollToBottom(false), 100); }
    });
    newSocket.on('new_message', (msg) => { setMessages(prev => [...prev, msg]); setTimeout(() => scrollToBottom(true), 100); });
    newSocket.on('user_typing', (d) => setOtherTyping(d.is_typing));
    newSocket.on('user_status', (d) => { if (d.userId === otherUserId) setOtherOnline(!!d.online); });
    newSocket.on('message_deleted', (d) => setMessages(prev => prev.filter(m => m.id !== d.messageId)));
    newSocket.on('incoming_call', (d) => { playRingtone(); setIncomingCall({ from: d.from, fromName: d.fromName, roomName: d.roomName }); });
    ...
    newSocket.on('error', () => { setSending(false); setUploading(false); });
    setSocket(newSocket);
  ...
  const handleScroll = () => { const el = scrollContainerRef.current; if (!el) return; setIsAtBottom(el.scrollHeight - el.scrollTop - el.clientHeight < 120); };
  ...
  const getOtherPerson = (s: Session) => user?.role === 'mentor'
    ? `${s.mentore_prenom ?? ''} ${s.mentore_nom ?? ''}`.trim()
    : `${s.mentor_prenom ?? ''} ${s.mentor_nom ?? ''}`.trim();

  const getOtherPhoto = (s: Session) => user?.role === 'mentor' ? s.mentore_photo_url : s.mentor_photo_url;
  ...
  const rows = useMemo(() => {
    ...
    messages.forEach((m, idx) => {
      ...
      const sp = prev && prev.expediteur_id === m.expediteur_id && (new Date(m.envoye_le).getTime() - new Date(prev.envoye_le).getTime()) < 300000;
      const sn = next && next.expediteur_id === m.expediteur_id && (new Date(next.envoye_le).getTime() - new Date(m.envoye_le).getTime()) < 300000;
      out.push({ kind: 'msg', key: m.id || `m-${idx}`, message: m, isFirst: !sp, isLast: !sn });
    });
    return out;
  }, [messages]);
  ...
  const sendTextMessage = (text: string) => {
    ...
    setMessages(prev => [...prev, { id: tid, contenu: text.trim(), expediteur_id: user?.id || '', envoye_le: new Date().toISOString(), nom: user?.nom || '', prenom: user?.prenom || '', tempId: tid }]);
    setNewMessage(''); setSending(true); scrollToBottom();
  ...
  const sendFile = async (file: File) => {
    try {
      ...
      if (result.success && socket) {
        ...
        const tid = Date.now().toString();
        setMessages(prev => [...prev, { id: tid, contenu: msgText, expediteur_id: user?.id || '', envoye_le: new Date().toISOString(), nom: user?.nom || '', prenom: user?.prenom || '', type_message: 'fichier', fichier_url: fileUrl, tempId: tid }]);
        scrollToBottom();
  ...
  const handleFilesSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Promise.all(files.map(f => f.type.startsWith('image/') ? new Promise<{url:string;name:string;type:string}>(resolve => { const r = new FileReader(); r.onloadend = () => resolve({url:r.result as string,name:f.name,type:f.type}); r.readAsDataURL(f); }) : Promise.resolve({url:'',name:f.name,type:f.type}))).then(p => setFilePreviews(prev => [...prev, ...p]));
    if (fileInputRef.current) fileInputRef.current.value = '';
  };

  const removeFile = (i: number) => { setSelectedFiles(prev => prev.filter((_,idx) => idx!==i)); setFilePreviews(prev => prev.filter((_,idx) => idx!==i)); };
  const handleTyping = () => {
    if (!socket) return;
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendAll(); } };

  const deleteMessage = async (id: string) => {
    if (!socket) return;
    const isTempId = !id.includes('-') || id.length < 30;
    if (isTempId) { setMessages(prev => prev.filter(m => m.id !== id)); setMenuOpenFor(null); return; }
    try { await messageAPI.deleteMessage(id); setMessages(prev => prev.filter(m => m.id !== id)); socket.emit('delete_message', { messageId: id, sessionId: selectedSessionId }); toast.success('Message supprimé'); }
    catch { toast.error('Erreur suppression'); }
    setMenuOpenFor(null);
  };

  const downloadFile = async (url: string, filename: string) => {
    try { const res = await fetch(url); const blob = await res.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = filename.replace(/[📎📷]/g,'').trim(); document.body.appendChild(a); a.click(); document.body.removeChild(a); }
    catch { window.open(url, '_blank'); }
  };

  const startCall = () => { if (!socket || !otherUserId) return; socket.emit('call_user',{to:otherUserId,roomName:selectedSessionId,callerName:`${user?.prenom} ${user?.nom}`}); setShowVideo(true); };
  const acceptCall = () => { if (incomingCall && socket) { stopRingtone(); socket.emit('accept_call',{to:incomingCall.from,roomName:incomingCall.roomName}); setShowVideo(true); setIncomingCall(null); } };
  const rejectCall = () => { if (incomingCall && socket) { stopRingtone(); socket.emit('reject_call',{to:incomingCall.from}); setIncomingCall(null); } };
  const endCall = () => { if (socket && otherUserId) socket.emit('end_call',{to:otherUserId}); setShowVideo(false); };
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className={`${isMobile && selectedSessionId ? 'hidden' : 'w-full md:w-[320px]'} flex-shrink-0 flex flex-col border-r`} style={{ borderColor: 'var(--border)' }}>
        <div className="px-5 pt-6 pb-4 flex-shrink-0">
          ...
          <div className="relative mb-3">
            ...
            <input type="text" placeholder={t('chat.search_placeholder')} value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-xl text-sm outline-none" style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }} />
          </div>
          <div className="flex gap-2">
            {(['all','active','completed'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} className="px-4 py-1.5 rounded-full text-sm font-medium transition-all" style={{ backgroundColor: filter === f ? 'var(--accent)' : 'var(--bg-secondary)', color: filter === f ? '#06231D' : 'var(--text-secondary)' }}>{f==='all'?'Tous':f==='active'?'Actifs':'Terminés'}</button>
            ))}
          </div>
        </div>
        <div className="flex-1 overflow-y-auto px-3 py-2">
          {filteredSessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              <MessageCircle className="w-12 h-12 mb-4" style={{ color: 'var(--text-tertiary)' }} />
              ...
              {user?.role === 'mentore' && !search && <Link href="/mentors" className="mt-5 px-6 py-3 rounded-xl text-sm font-semibold" style={{ backgroundColor: 'var(--accent)', color: '#06231D' }}>Trouver un mentor</Link>}
            </div>
          ) : (
            filteredSessions.map((s, idx) => {
              const other = getOtherPerson(s), otherPhoto = getOtherPhoto(s);
              ...
              return (
                <motion.div key={s.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.03 }}>
                  <div onClick={() => setSelectedSessionId(s.id)} className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all cursor-pointer" style={{ backgroundColor: isSelected ? 'var(--accent-soft)' : 'transparent', border: isSelected ? '1px solid var(--accent)' : '1px solid transparent' }}>
                    <div className="relative flex-shrink-0"><Avatar name={other||'?'} photoUrl={otherPhoto} size={48} /><div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 ${status.dot}`} style={{ borderColor: 'var(--bg-primary)' }} /></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5"><p className="font-semibold text-sm truncate" style={{ color: 'var(--text-primary)' }}>{other||'Inconnu'}</p><span className="text-xs flex-shrink-0 ml-2" style={{ color: 'var(--text-tertiary)' }}>{formatDate(s.date_debut)}</span></div>
                      <div className="flex items-center justify-between"><p className="text-sm truncate" style={{ color: 'var(--text-secondary)' }}>{s.sujet}</p><span className="text-[10px] px-2 py-0.5 rounded-full flex-shrink-0 ml-2 font-medium" style={{ backgroundColor: `${status.color}15`, color: status.color }}>{status.label}</span></div>
                    </div>
                  </div>
                </motion.div>
              );
        ...
        {selectedSession ? (
          <>
            ...
            <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-4 py-4">
              {chatLoading ? (
                <div className="flex items-center justify-center h-full"><div className="w-8 h-8 border-3 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--accent)', borderTopColor: 'transparent' }} /></div>
              ) : rows.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center gap-3"><MessageCircle className="w-12 h-12" style={{ color: 'var(--text-tertiary)' }} /><p className="font-semibold" style={{ color: 'var(--text-primary)' }}>Aucun message</p><p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>Dites bonjour !</p></div>
              ) : (
                rows.map((row) => {
                  if (row.kind === 'date') return <div key={row.key} className="flex items-center gap-3 my-6"><div className="flex-1 h-px" style={{ backgroundColor: 'var(--border)' }} /><span className="text-xs font-medium px-4 py-1 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-tertiary)' }}>{row.label}</span><div className="flex-1 h-px" style={{ backgroundColor: 'var(--border)' }} /></div>;
                  const m = row.message, isOwn = m.expediteur_id === user?.id;
                  const fullUrl = getFullUrl(m.fichier_url || ''), isImg = isImageFile(fullUrl), isFile = m.type_message === 'fichier' && m.fichier_url;
                  const radius = isOwn ? '16px 16px 4px 16px' : '16px 16px 16px 4px';
                  return (
                    <motion.div key={row.key} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`flex group ${isOwn ? 'justify-end' : 'justify-start'} mb-1`}>
                      <div className={`flex items-end gap-2 max-w-[75%] ${isOwn ? 'flex-row-reverse' : ''}`}>
                        {!isOwn && <div style={{ visibility: row.isLast ? 'visible' : 'hidden' }}><Avatar name={otherUserName} photoUrl={otherUserPhoto} size={28} /></div>}
                        <div className="relative px-4 py-2.5" style={{ background: isOwn ? 'var(--accent)' : 'var(--card-bg)', color: isOwn ? '#06231D' : 'var(--text-primary)', border: isOwn ? 'none' : '1px solid var(--border)', borderRadius: radius, boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                          {isFile ? (
                            <div onClick={() => isImg ? setSelectedImage(fullUrl) : downloadFile(fullUrl, m.contenu)} className="cursor-pointer">
                              {isImg ? <img src={fullUrl} alt="" className="max-w-[200px] max-h-[180px] rounded-lg object-cover" /> : <div className="flex items-center gap-3 p-2 rounded-lg" style={{ backgroundColor: isOwn ? 'rgba(6,35,29,0.08)' : 'var(--bg-secondary)' }}><File className="w-5 h-5" /><span className="text-sm truncate">{m.contenu.replace(/[📎📷]/g,'').trim()}</span><Download className="w-4 h-4 opacity-50" /></div>}
                            </div>
                          ) : <p className="text-sm break-words whitespace-pre-wrap">{m.contenu}</p>}
                          <div className={`flex items-center gap-1 mt-1 ${isOwn ? 'justify-end' : 'justify-start'}`}><span className="text-[10px] opacity-60">{new Date(m.envoye_le).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</span>{isOwn && (m.lu ? <CheckCheck className="w-3 h-3 opacity-60" /> : <Check className="w-3 h-3 opacity-40" />)}</div>
                          {isOwn && !m.tempId && (
                            <button onClick={(e) => { e.stopPropagation(); setMenuOpenFor(menuOpenFor === m.id ? null : m.id); }} className="absolute -left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg hover:bg-[var(--bg-secondary)]" style={{ color: 'var(--text-tertiary)' }}><MoreVertical className="w-4 h-4" /></button>
                          )}
                          {isOwn && !m.tempId && menuOpenFor === m.id && (
                            <div ref={menuRef} className="absolute right-0 top-full mt-1 rounded-lg shadow-lg overflow-hidden z-30 min-w-[140px]" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)' }}>
                              <button onClick={(e) => { e.stopPropagation(); deleteMessage(m.id); }} className="flex items-center gap-2 px-4 py-2.5 text-sm w-full transition-colors hover:bg-[var(--bg-secondary)]" style={{ color: 'var(--danger)' }}><Trash2 className="w-4 h-4" />Supprimer le message</button>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {otherTyping && <div className="flex items-end gap-2 mt-1"><Avatar name={otherUserName} photoUrl={otherUserPhoto} size={28} /><div className="px-4 py-2.5 rounded-lg" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)' }}><div className="flex gap-1.5"><span className="w-2 h-2 rounded-full animate-bounce" style={{ backgroundColor: 'var(--text-tertiary)', animationDelay: '0ms' }} /><span className="w-2 h-2 rounded-full animate-bounce" style={{ backgroundColor: 'var(--text-tertiary)', animationDelay: '150ms' }} /><span className="w-2 h-2 rounded-full animate-bounce" style={{ backgroundColor: 'var(--text-tertiary)', animationDelay: '300ms' }} /></div></div></div>}
              <div ref={messagesEndRef} />
            </div>

            {!isAtBottom && <button onClick={() => scrollToBottom(true)} className="absolute bottom-20 right-4 w-10 h-10 rounded-xl flex items-center justify-center shadow-lg z-10" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)' }}><ChevronDown className="w-5 h-5" style={{ color: 'var(--text-secondary)' }} /></button>}
                ...
                <div className="mb-3 p-3 rounded-xl flex flex-wrap gap-3" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                  {filePreviews.map((p,i) => (
                    <div key={i} className="relative group">
                      {p.url ? <img src={p.url} className="w-16 h-16 rounded-lg object-cover" /> : <div className="w-16 h-16 rounded-lg flex items-center justify-center" style={{ backgroundColor: 'var(--accent-soft)' }}><File className="w-8 h-8" style={{ color: 'var(--accent)' }} /></div>}
                      <button onClick={() => removeFile(i)} className="absolute -top-2 -right-2 rounded-full p-1 shadow-md" style={{ backgroundColor: 'var(--danger)', color: '#fff' }}><X className="w-3 h-3" /></button>
                    </div>
                  ))}
                </div>
              )}
              <div className="flex items-end gap-2">
                <button onClick={() => fileInputRef.current?.click()} className="p-2 rounded-lg hover:bg-[var(--bg-secondary)]" style={{ color: 'var(--text-secondary)' }}><Paperclip className="w-5 h-5" /></button>
                <div className="relative" ref={emojiRef}>
                  <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="p-2 rounded-lg hover:bg-[var(--bg-secondary)]" style={{ color: 'var(--text-secondary)' }}><Smile className="w-5 h-5" /></button>
                  {showEmojiPicker && <div className="absolute bottom-12 left-0 z-50 shadow-xl rounded-2xl overflow-hidden"><EmojiPicker onEmojiClick={(e:any) => { setNewMessage(prev => prev + e.emoji); setShowEmojiPicker(false); }} theme={theme==='dark'?'dark' as any:'light' as any} /></div>}
                </div>
                <input ref={fileInputRef} type="file" accept="image/*,application/pdf,.txt" multiple className="hidden" onChange={handleFilesSelect} />
                <textarea ref={textareaRef} value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={handleKeyPress} onKeyUp={handleTyping} placeholder="Écrivez votre message..." rows={1} className="flex-1 resize-none border rounded-xl px-4 py-2.5 outline-none" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border)', color: 'var(--text-primary)', minHeight: '44px', maxHeight: '120px' }} />
                <button onClick={sendAll} disabled={!hasContent} className="w-11 h-11 rounded-xl flex items-center justify-center transition-all disabled:opacity-40 hover:scale-105 active:scale-95" style={{ backgroundColor: hasContent ? 'var(--accent)' : 'var(--bg-secondary)', color: hasContent ? '#06231D' : 'var(--text-tertiary)' }}>
                  {sending || uploading ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <Send className="w-4 h-4" />}
                </button>
              </div>
            </div>
          </>
      ...
      {/* Visio */}
      {showVideo && <SimpleJitsi roomName={selectedSessionId!} userName={`${user?.prenom} ${user?.nom}`} contactName={otherUserName} contactId={otherUserId||undefined} onClose={endCall} />}
      {incomingCall && <IncomingCallModal callerName={incomingCall.fromName} onAccept={acceptCall} onReject={rejectCall} />}
    </div>
  );
}