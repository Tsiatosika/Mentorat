    ...
    return (
      <div className="min-h-screen p-6" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-5xl mx-auto py-4">
          <div className="skeleton-block h-3 w-40 rounded-full mb-3" />
          <div className="skeleton-block h-10 w-72 rounded-xl mb-8" />
          <div className="space-y-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="rounded-2xl p-5 skeleton-card" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)', animationDelay: `${i * 0.06}s` }}>
                <div className="flex items-start gap-4">
                  <div className="skeleton-block w-11 h-11 rounded-xl flex-shrink-0" />
                  <div className="flex-1 space-y-2">
                    <div className="skeleton-block h-3 w-1/3 rounded-md" />
                    <div className="skeleton-block h-5 w-2/3 rounded-md" />
                    <div className="skeleton-block h-3 w-full rounded-md" />
                  </div>
                </div>
              </div>
            ))}
        ...
        <style jsx global>{`
          ...
          .skeleton-card { opacity: 0; animation: skeletonCardIn 0.4s ease forwards; }
    ...
    <div className="sessions-page min-h-screen relative overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="mb-10 fade-in-up">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <h1 className="font-display text-4xl md:text-5xl font-semibold mb-3 shimmer-text"
                style={{ lineHeight: 1.1, letterSpacing: '-0.02em' }}>
                {t('sessions.title')}
              </h1>
              <p style={{ color: 'var(--text-secondary)', fontSize: '15px' }}>
                {t('sessions.page_description')}
              </p>
            </div>
            ...
            {user?.role === 'mentore' && (
              <Link href="/mentors"
                ...
                <Plus className="w-4 h-4 plus-icon" /> {t('sessions.new_session')}
              </Link>
            )}
          </div>
          ...
          <div className="flex gap-3 mt-6 flex-wrap">
            {[
              ...
              <div key={stat.label} className="flex items-center gap-2 px-3 py-1.5 rounded-lg stat-chip-in stat-chip"
                style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', animationDelay: `${0.1 + i * 0.06}s` }}>
        ...
        <div className="mb-8 fade-in-up" style={{ animationDelay: '0.12s' }}>
          <div className="flex gap-1 p-1 rounded-2xl flex-wrap"
            style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', display: 'inline-flex' }}>
            {FILTERS.map(f => {
              const cnt = f.key === 'confirmee' ? counts.confirmee + counts.en_cours : counts[f.key] ?? 0;
              const active = filter === f.key;
              return (
                <button key={f.key} onClick={() => setFilter(f.key)}
                  className="filter-pill px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200"
                  style={active
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center fade-in-up">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-5 empty-icon-float"
            ...
            <p className="text-sm mb-6" style={{ color: 'var(--text-tertiary)' }}>
              {user?.role === 'mentore' ? t('sessions.empty_mentee') : t('sessions.empty_mentor')}
            </p>
            {user?.role === 'mentore' && (
              <Link href="/mentors"
                className="sessions-btn-primary px-5 py-2.5 rounded-xl font-semibold text-sm"
                style={{ backgroundColor: 'var(--accent)', color: '#FFFFFF' }}>
          ...
          <div className="space-y-4" key={filter}>
            {filtered.map((s, idx) => {
              ...
              return (
                <div
                  key={s.id}
                  className="session-card-in group relative rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1"
                  style={{
                ...
                >
                  ...
                  {isActive && (
                    <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl session-active-bar"
                      style={{ backgroundColor: 'var(--success)' }} />
                  )}

                  {/* Hover glow */}
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                    style={{ background: `radial-gradient(ellipse at 50% 0%, ${st.bg} 0%, transparent 70%)` }} />

                  <div className="relative flex flex-wrap items-start gap-4 p-5">

                    {/* Icône calendrier */}
                    <div className="rounded-xl p-3 flex-shrink-0 session-icon-wrap"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        ...
                        {hasReviewed && (
                          <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full reviewed-badge"
                            style={{ backgroundColor: 'var(--warm-soft)', color: 'var(--warm-text-on-soft)', border: '1px solid var(--border)' }}>
                      ...
                      {/* Titre */}
                      <h3 className="text-xl font-bold mb-1.5 leading-tight font-display"
                    ...
                    <div className="flex flex-col items-end gap-3 flex-shrink-0">
                      ...
                      {/* Badge statut */}
                      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full status-badge"
                      ...
                      <div className="flex gap-2 flex-wrap justify-end">
                        ...
                        {s.statut === 'en_attente' && isMentor && (
                          <button onClick={() => handleConfirm(s.id)}
                            ...
                            style={{ backgroundColor: 'var(--success-soft)', color: 'var(--success)', border: '1px solid var(--border)' }}>
                            <CheckCircle className="w-3.5 h-3.5" /> {t('sessions.confirm')}
                          </button>
                        )}
                        {/* Démarrer (mentor, confirmee) */}
                        {s.statut === 'confirmee' && isMentor && (
                          <button onClick={() => handleStart(s.id)}
                            className="session-action-btn flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold"
                            style={{ backgroundColor: 'var(--info-soft)', color: 'var(--info)', border: '1px solid var(--border)' }}>
                        ...
                        {(s.statut === 'en_attente' || s.statut === 'confirmee') && (
                          <button onClick={() => handleCancel(s.id)}
                            className="session-action-btn flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold"
                            style={{ backgroundColor: 'var(--danger-soft)', color: 'var(--danger)', border: '1px solid var(--border)' }}>
                            <XCircle className="w-3.5 h-3.5" /> {t('sessions.cancel')}
                          </button>
                        )}
                        {/* Visio */}
                        {s.lien_visio && (s.statut === 'confirmee' || s.statut === 'en_cours') && (
                          <a href={s.lien_visio} target="_blank" rel="noopener noreferrer"
                            className="session-action-btn visio-btn flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold"
                            style={{ backgroundColor: 'var(--success-soft)', color: 'var(--success)', border: '1px solid var(--border)' }}>
                        ...
                        {canReview && (
                          <button
                            ...
                            className="session-action-btn rate-btn flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold"
                            style={{ backgroundColor: 'var(--warm-soft)', color: 'var(--warm-text-on-soft)', border: '1px solid var(--border)' }}>
      ...
      {reviewModal && (
        <ReviewModal
          ...
          onSubmitted={() => { load(); setTimeout(() => setReviewModal(null), 1500); }}
        />
      )}
      ...
      <style jsx global>{`
        ...
        @keyframes sessionsFloat1 {
          0%, 100% { transform: translate(0, 0) scale(1); }
          50% { transform: translate(-30px, 25px) scale(1.07); }
        }
        @keyframes sessionsFloat2 {
          0%, 100% { transform: translate(0, 0) scale(1); }
          50% { transform: translate(25px, -20px) scale(1.05); }
        }

        .sessions-particle {
          animation: sessionsParticlePulse ease-in-out infinite;
        }
        @keyframes sessionsParticlePulse {
          0%, 100% { opacity: 0.15; }
          50% { opacity: 0.5; }
        }
        ...
        .fade-in-up {
          ...
          animation: sessionsFadeUp 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        @keyframes sessionsFadeUp {
        ...
        .shimmer-text {
          ...
          color: transparent;
          animation: shimmerTextSlide 5s ease-in-out 1s infinite;
        }
        ...
        .stat-chip-in {
          ...
          animation: sessionsChipIn 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }
        @keyframes sessionsChipIn {
          to { opacity: 1; transform: scale(1); }
        }
        .stat-chip { transition: transform 0.2s ease, box-shadow 0.2s ease; }
        .stat-chip:hover { transform: translateY(-2px); box-shadow: 0 6px 14px rgba(0,0,0,0.08); }

        .filter-pill { position: relative; }
        .filter-pill:hover { filter: brightness(1.05); }

        .session-card-in {
          opacity: 0;
          transform: translateY(16px);
          animation: sessionsCardIn 0.45s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        @keyframes sessionsCardIn {
          to { opacity: 1; transform: translateY(0); }
        }

        .session-active-bar {
          animation: sessionsPulseBar 1.6s ease-in-out infinite;
        }
        @keyframes sessionsPulseBar {
          0%, 100% { opacity: 1; }
          50% { opacity: 0.45; }
        }

        .session-icon-wrap { transition: transform 0.3s cubic-bezier(0.34,1.56,0.64,1); }
        .group:hover .session-icon-wrap { transform: scale(1.08) rotate(-4deg); }

        .status-badge { transition: transform 0.2s ease; }
        .group:hover .status-badge { transform: scale(1.04); }

        .reviewed-badge { animation: reviewedPop 0.4s cubic-bezier(0.34,1.56,0.64,1); }
        @keyframes reviewedPop { from { opacity: 0; transform: scale(0.7); } to { opacity: 1; transform: scale(1); } }

        .plus-icon { transition: transform 0.25s ease; }
        .sessions-btn-primary:hover .plus-icon { transform: rotate(90deg); }

        .empty-icon-float { animation: emptyIconFloat 3s ease-in-out infinite; }
        ...
        .rate-btn:hover { animation: rateShimmer 0.5s ease; }
        @keyframes rateShimmer {
          0%, 100% { filter: brightness(1); }
          50% { filter: brightness(1.2); }
        }
        .visio-btn:hover { box-shadow: 0 4px 12px rgba(34,197,94,0.25); }
        .sessions-btn-primary:active,
        .session-action-btn:active {
          transform: scale(0.96);
        }

        @media (prefers-reduced-motion: reduce) {
          .sessions-orb, .sessions-particle, .fade-in-up, .stat-chip-in,
          .session-card-in, .session-active-bar, .shimmer-text, .session-icon-wrap,
          .status-badge, .reviewed-badge, .plus-icon, .empty-icon-float {
