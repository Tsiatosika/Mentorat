...
function ReportsContent() {
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 pt-12 pb-8">
          ...
          <div className="mb-8 p-6 rounded-2xl report-detail-card" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)' }}>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center report-detail-icon" style={{ backgroundColor: 'var(--accent-soft)' }}>
                <FileText className="w-5 h-5" style={{ color: 'var(--accent)' }} />
              </div>
              <div>
                <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{session?.sujet || t('reports.session_report')}</h1>
                {session && <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>{formatDateBadge(session.date_debut)}</p>}
              </div>
            </div>
            {session?.description && <p className="text-sm mt-2" style={{ color: 'var(--text-secondary)' }}>{session.description}</p>}
          </div>

          {!hasRapports ? (
            <div className="text-center py-12 px-6 rounded-2xl report-empty-card" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)' }}>
              <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-4 report-empty-icon" style={{ backgroundColor: 'var(--accent-soft)' }}>
              ...
              <p className="text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>{t('reports.no_reports_desc')}</p>
              <button onClick={() => handleGenerateRapport(sessionIdFromUrl)} disabled={generating === sessionIdFromUrl} className="px-6 py-3 rounded-xl font-semibold text-sm transition-all disabled:opacity-50 hover-generate-btn" style={{ backgroundColor: 'var(--accent)', color: '#FFFFFF' }}>
            ...
            <div>
              ...
              <div className="space-y-4">
                {rapports.map((rapport) => (
                  <div key={rapport.id} className="p-5 rounded-xl transition-all report-file-card" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)' }}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center report-file-icon" style={{ backgroundColor: 'var(--success-soft)' }}>
                        ...
                        <div>
                          ...
                          <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{new Date(rapport.genere_le).toLocaleString(language==='fr'?'fr-FR':'en-GB',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})}</p>
                        </div>
                      </div>
                      <button onClick={() => handleDownloadRapport(sessionIdFromUrl)} className="px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 hover-download-btn" style={{ backgroundColor: 'var(--accent-soft)', color: 'var(--accent-text-on-soft)' }}>
                        <Download className="w-4 h-4" />{t('reports.download')}
                      </button>
                    </div>
                  </div>
                ))}
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)' }}>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-4">
            {filtered.map((session) => {
              ...
              return (
                <div key={session.id} className="p-5 rounded-xl transition-all report-list-card" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border)', transform: isHovered ? 'translateY(-4px)' : 'translateY(0)', boxShadow: isHovered ? '0 12px 28px rgba(0,0,0,0.1)' : 'var(--shadow-card)' }} onMouseEnter={() => setHoveredCard(session.id)} onMouseLeave={() => setHoveredCard(null)}>
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <span className="text-xs px-3 py-1 rounded-full" style={{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-tertiary)' }}>{formatDateBadge(session.date_debut)}</span>
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center report-list-icon" style={{ backgroundColor: 'var(--accent-soft)', transform: isHovered ? 'scale(1.1) rotate(-6deg)' : 'scale(1) rotate(0deg)' }}>
                      <FileText className="w-5 h-5" style={{ color: 'var(--accent)' }} />
                    </div>
                  </div>
                  <h3 className="text-lg font-bold mb-2" style={{ color: isHovered ? 'var(--accent)' : 'var(--text-primary)' }}>{session.sujet}</h3>
                  ...
                  <div style={{ borderTop: '1px solid var(--border)' }} className="pt-4 flex items-center justify-between">
                    <Link href={`/reports?sessionId=${session.id}`} className="flex items-center gap-1.5 text-sm font-semibold hover:opacity-80 hover-link-arrow" style={{ color: 'var(--accent)' }}>
      ...
      <style jsx global>{`
        .reports-orb { position: absolute; border-radius: 9999px; filter: blur(70px); opacity: 0.4; will-change: transform; }
        ...
        .report-file-icon { transition: all 0.3s ease; }
        .report-file-card:hover .report-file-icon { transform: scale(1.1); }
