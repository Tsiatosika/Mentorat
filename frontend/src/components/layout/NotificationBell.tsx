        ...
        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              {unreadCount > 0 && (
                <button onClick={markAllAsRead} style={{ fontSize: '11px', color: '#4f46e5', background: 'none', border: 'none', cursor: 'pointer' }}>
                  Tout marquer lu
                </button>
              )}
            </div>
                ...
                notifications.map((notif) => (
                  ...
                  >
                    <div style={{ display: 'flex', gap: '10px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <p style={{ fontSize: '12px', fontWeight: 500, color: '#1f2937', marginBottom: '2px' }}>{notif.titre}</p>
                        <p style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px' }}>{notif.message}</p>
                        <p style={{ fontSize: '10px', color: '#9ca3af' }}>
                          {new Date(notif.created_at).toLocaleDateString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
                      {!notif.lue && <div style={{ width: '8px', height: '8px', background: '#4f46e5', borderRadius: '50%', marginTop: '6px' }} />}
                    </div>
                  </div>
                ))
              )}
